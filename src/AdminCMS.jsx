...
export default function AdminCMS({
  ...
  // ✅ [NEW] 추천인 코드 관리 (users 컬렉션)
  const [refUserId, setRefUserId] = useState("");
  const [refCodeInput, setRefCodeInput] = useState("");
  const [refLookup, setRefLookup] = useState(null); // { id, refCode }

  useEffect(() => {
    // initialMember가 memo이긴 하지만, regionData 바뀌면 초기화 필요할 수 있어 안전하게
    setNewM((prev) => (prev?.region ? prev : initialMember));
  }, [initialMember]);
  ...
  // ✅ 공통: 서버 즉시 저장 헬퍼
  const saveNow = async (updates, successMsg) => {
    try {
      setLoading(true);
      const ok = await syncToFirebase(updates);
      if (ok && successMsg) alert(successMsg);
      return ok;
    } finally {
      setLoading(false);
    }
  ...
  const saveMember = async () => {
    ...
    const ok = await saveNow({ members: updatedMembers }, "매니저 정보가 서버에 즉시 반영되었습니다! ✅");
  ...
  const saveVideoToGallery = async () => {
    ...
    const ok = await saveNow({ videos: updatedVideos }, "갤러리 영상이 서버에 반영되었습니다! ✅");
    if (ok) {
      setTempVideoUrl("");
      setVideoDesc("");
    }
  ...
  const handleFileProcess = async (e, mode, callback) => {
    ...
    try {
      ...
      else if (mode === "managerImg") {
        // newM 업데이트는 callback에서 했고, 실제 저장은 saveMember에서 처리
      } else if (mode === "managerVid") {
        // 동일
      } else if (mode === "galleryVid") {
        // 갤러리 저장은 saveVideoToGallery에서 처리
      }
  ...
  // ✅ 시스템 설정(텔레그램/비밀번호) 즉시 저장
  const handleSaveSystemSettings = async () => {
    await saveNow(
      { telegramLink: telegramLink, adminPw: adminPw },
      "텔레그램 아이디 및 관리자 비밀번호가 즉시 저장되었습니다! ✅"
    );
  };

  // ✅ 로고 위치/크기 즉시 저장 (버튼)
  const handleSaveLogoLayout = async () => {
    await saveNow({ logoSize, logoPos }, "로고 크기/위치가 서버에 저장되었습니다! ✅");
  };
  ...
  const lookupUserRefCode = async () => {
    const uid = String(refUserId || "").trim();
    if (!uid) return alert("회원 ID를 입력하세요.");
    ...
    try {
      ...
      if (!snap.exists()) {
        setRefLookup(null);
        setRefCodeInput("");
        return alert("해당 ID의 users 문서가 없습니다. (먼저 회원가입이 되어 있어야 합니다)");
      }
      const data = snap.data() || {};
      const currentCode = data.refCode || "";
      setRefLookup({ id: uid, refCode: currentCode });
      setRefCodeInput(currentCode);
      alert("조회 완료 ✅");
    } catch (e) {
      console.error(e);
      alert("조회 실패");
  ...
  const saveUserRefCode = async () => {
    const uid = String(refUserId || "").trim();
    const code = String(refCodeInput || "").trim();
    if (!uid) return alert("회원 ID를 입력하세요.");
    if (!code) return alert("추천인 코드를 입력(또는 생성)하세요.");
    ...
    try {
      ...
      if (!snap.exists()) return alert("해당 ID의 users 문서가 없습니다. (먼저 회원가입 필요)");

      // ✅ 여기서 users/{id}.refCode 저장해야 회원가입 추천인 검증이 즉시 먹음
      await updateDoc(ref, {
        refCode: code,
        refCodeUpdatedAt: Date.now(),
      });

      setRefLookup({ id: uid, refCode: code });
      alert("추천인 코드가 저장되었습니다! (즉시 적용) ✅");
    } catch (e) {
      console.error(e);
      alert("저장 실패");
    } finally {
    ...
    <div style={cmsStyles.cms}>
      ...
      <div style={{ padding: "20px" }}>
        ...
        <div style={cmsStyles.sectionBox}>
          ...
          <div style={cmsStyles.fieldGroup}>
            ...
            <input
              ...
              onChange={(e) =>
                handleFileProcess(e, "heroImg", (url) => setHero({ ...hero, imageSrc: url, mode: "image" }))
              }
            />
            {hero.imageSrc && (
              <img
          ...
          <div style={cmsStyles.fieldGroup}>
            ...
            <button
              onClick={handleSaveLogoLayout}
              style={{ ...cmsStyles.modalOpenBtn, marginTop: 10, background: "#4CAF50" }}
            >
              로고 크기/위치 즉시저장
            </button>
          </div>
        ...
        <div style={cmsStyles.sectionBox}>
          ...
          <button
            onClick={() => openIndependent && openIndependent()}
        ...
        {/* ✅ SECTION 4: 추천인 코드(초대코드) 관리 - users 컬렉션 */}
        <div style={{ ...cmsStyles.sectionBox, borderColor: "#4CAF50" }}>
          <label style={{ ...cmsStyles.sectionLabel, color: "#4CAF50" }}>❹ 추천인 코드(초대코드) 관리</label>

          <div style={cmsStyles.fieldGroup}>
            <label style={cmsStyles.fieldLabel}>회원 ID (users/{`{id}`})</label>
            <input
              type="text"
              style={cmsStyles.textInput}
              placeholder="예: testuser123"
              value={refUserId}
              onChange={(e) => setRefUserId(e.target.value)}
            />
            <button
              onClick={lookupUserRefCode}
              style={{ ...cmsStyles.modalOpenBtn, background: "#4CAF50", marginTop: 8 }}
            >
              조회
            </button>
            ...
            {refLookup && (
              <p style={{ fontSize: 11, color: "#aaa", marginTop: 8 }}>
                현재 코드: <b style={{ color: "#fff" }}>{refLookup.refCode || "(없음)"}</b>
              </p>
            )}
          </div>

          <div style={cmsStyles.fieldGroup}>
            <label style={cmsStyles.fieldLabel}>추천인 코드(refCode)</label>
            <input
              type="text"
              style={cmsStyles.textInput}
              placeholder="예: DAISY123"
              value={refCodeInput}
              onChange={(e) => setRefCodeInput(e.target.value.toUpperCase())}
            />
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button
                onClick={() => setRefCodeInput(genRefCode())}
                style={{ ...cmsStyles.modalOpenBtn, background: "#333", color: "#fff" }}
              >
                랜덤 생성
              </button>
              <button
                onClick={saveUserRefCode}
                style={{ ...cmsStyles.modalOpenBtn, background: "#4CAF50" }}
              >
                저장(즉시 적용)
              </button>
            </div>
            <p style={{ fontSize: 10, color: "#777", marginTop: 8, lineHeight: 1.35 }}>
              * 여기서 저장하면 <b>users/{`{id}`}.refCode</b>가 바뀌고, 회원가입 추천인 코드 검증에 <b>즉시 반영</b>됩니다.
            </p>
          </div>
        </div>
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 10, marginTop: 10 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleFileProcess(e, "managerImg", (url) => setNewM({ ...newM, img: url }))}
                  />
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    type="file"
                    accept="video/*"
                    onChange={(e) => handleFileProcess(e, "managerVid", (url) => setNewM({ ...newM, video: url }))}
                  />
            ...
            <div style={modalStyles.list}>
              {members.map((m) => (
                <div key={m.id} style={modalStyles.listItem}>
                  ...
                  <div>
                    <button onClick={() => startEdit(m)} style={modalStyles.editBtn}>
                      수정
                    </button>
                    <button
                      onClick={async () => {
          ...
          <div id="video-modal-scroll" style={{ ...modalStyles.container, border: "2px solid #2196F3" }}>
            ...
            <div style={modalStyles.formBox}>
              ...
              <input
                type="file"
                accept="video/*"
                onChange={(e) => handleFileProcess(e, "galleryVid", (url) => setTempVideoUrl(url))}
              />
